...
int main() {
    ...
    for (i = 0; i < round; i++) {
        if (i == 0)
            ...
            sha1(md5_out, sha1_out, strlen((char *)md5_out));

        blake2b(blake_out, 64, NULL, 0, sha1_out, strlen((char *)sha1_out));

        sha256(blake_out, strlen((char *)blake_out), sha256_out);

        md5(sha256_out, strlen((char *)sha256_out), md5_out);
    }
    end = clock();

    dura = (double) (end - start) / CLOCKS_PER_SEC;
    printf("Setup time (Case II): %f seconds.\n", dura);
